...
public class PluginUtils {
    ...
    public static File getPluginFile(String name) {
        for (File pluginFile : Files.file("plugins").listFiles()) {
            if (pluginFile.getName().endsWith(".jar")) {
                try {
                    PluginDescriptionFile desc = TabooLib.getPlugin().getPluginLoader().getPluginDescription(pluginFile);
    ...
    public static void enableAll() {
        Arrays.stream(Bukkit.getPluginManager().getPlugins()).filter(io.izzel.taboolib.util.plugin.PluginUtils::isIgnored).forEach(io.izzel.taboolib.util.plugin.PluginUtils::enable);
    }

    public static void disableAll() {
        Arrays.stream(Bukkit.getPluginManager().getPlugins()).filter(io.izzel.taboolib.util.plugin.PluginUtils::isIgnored).forEach(io.izzel.taboolib.util.plugin.PluginUtils::disable);
    }
    ...
    public static boolean isIgnored(Plugin plugin) {
        return plugin.getName().equalsIgnoreCase("TabooLib");
    }
            ...
            if (!pluginFile.exists()) {
                for (File plugin : Objects.requireNonNull(pluginDir.listFiles())) {
                    if (plugin.getName().endsWith(".jar")) {
                        try {
                            PluginDescriptionFile desc = TabooLib.getPlugin().getPluginLoader().getPluginDescription(plugin);
                            if (desc.getName().equalsIgnoreCase(name)) {
                                pluginFile = plugin;
                                break;
                            }
    ...
    public static void reloadAll() {
        Arrays.stream(Bukkit.getPluginManager().getPlugins(), 0, Bukkit.getPluginManager().getPlugins().length).filter(io.izzel.taboolib.util.plugin.PluginUtils::isIgnored).forEach(io.izzel.taboolib.util.plugin.PluginUtils::reload);
    }
    ...
    public static PluginUnloadState unload(Plugin plugin) {
        ...
        if (cl instanceof URLClassLoader) {
            try {
                ...
                Logger.getLogger(io.izzel.taboolib.util.plugin.PluginUtils.class.getName()).log(Level.SEVERE, null, var13);
            }
        }
